...
const App: React.FC = () => {
  ...
  const startAnalysis = async () => {
    ...
    setState(prev => ({ ...prev, step: 'analyzing-context', error: null, processingStatus: 'Deconstructing Narrative Arc & Scene Roadmap...' }));
    try {
  ...
  const executeEliteProtocol = async () => {
    ...
    setState(prev => ({ 
      ...
      processingStatus: 'Engineering Veo3.1 Cinematic Prompts...',
      error: null 
    }));
    ...
    try {
      ...
      setState(prev => ({ ...prev, processingStatus: 'Synthesizing Elite English Masterclass...' }));
      ...
      setState(prev => ({
        ...
        result: {
          ...pass1,
          veoPrompt: refinedPrompt // Updating the Veo prompt with the refined version
        }
      }));
    } catch (err: any) {
    ...
    <div className="min-h-screen flex flex-col items-center p-4 md:p-12 bg-[#020617] text-slate-200">
      <header className="w-full max-w-7xl flex flex-col md:flex-row justify-between items-center mb-16 gap-12 border-b border-white/5 pb-12">
        <div className="text-center md:text-left">
          <h1 className="text-[6rem] md:text-[8rem] font-black font-serif italic tracking-tighter gold-glow text-white animate-pulse leading-none select-none">
            AL-NOKHBA
          </h1>
          <div className="flex items-center gap-3 mt-4">
            <div className="h-0.5 w-16 bg-yellow-600/40"></div>
            <p className="text-yellow-600 font-bold uppercase text-[12px] tracking-[1.5em] italic">Veo3.1 Elite Director v7.0</p>
          </div>
        </div>
      </header>
      ...
      <main className="w-full max-w-7xl glass rounded-[4rem] shadow-[0_80px_150px_rgba(0,0,0,0.9)] overflow-hidden p-8 md:p-20 relative ring-1 ring-white/5 bg-gradient-to-br from-[#020617] via-[#020617] to-[#0a0a0f]">
          ...
          <div className="space-y-16 py-12">
            <div className="relative border-2 border-dashed border-white/5 rounded-[4rem] p-20 md:p-32 text-center group hover:border-yellow-600/30 transition-all duration-1000 cursor-pointer bg-slate-950/20 shadow-2xl">
              ...
              <div className="space-y-10 relative z-10">
                ...
                <p className="text-slate-500 text-xs tracking-[0.5em] uppercase">Upload full video for narrative mapping</p>
              </div>
            </div>
            <Button className="w-full py-10 md:py-12 text-2xl md:text-3xl font-black rounded-[3rem] bg-white text-slate-950 hover:bg-slate-200 tracking-tighter font-serif italic shadow-3xl" disabled={!state.contextVideo} onClick={startAnalysis}>MAP NARRATIVE & CONTEXT</Button>
          </div>
        )}
        ...
        {state.step === 'upload-scene' && state.contextData && (
          <div className="space-y-16 animate-fadeIn">
            {/* Scene Roadmap Preview */}
            <div className="bg-slate-950/40 p-10 rounded-[3rem] border border-white/10 space-y-8 shadow-2xl mb-12">
               <h4 className="text-[12px] font-black text-yellow-600 uppercase tracking-[1em] italic">Full Video Scene Roadmap</h4>
               <div className="overflow-x-auto custom-scrollbar pb-6">
                 <div className="flex gap-6 w-max">
                   {state.contextData.sceneRoadmap.map((scene) => (
                     <div key={scene.sceneId} className="w-64 p-6 bg-black/40 rounded-3xl border border-white/5 hover:border-yellow-600/30 transition-all flex flex-col gap-3">
                       <span className="text-[10px] text-slate-500 uppercase tracking-widest font-black">SCENE {scene.sceneId}</span>
                       <p className="text-sm font-serif italic text-white/80 line-clamp-3">{scene.description}</p>
                       <div className="mt-auto pt-4 border-t border-white/5">
                          <span className="text-[8px] text-indigo-400 uppercase tracking-wider">{scene.estimatedDuration}</span>
                       </div>
                     </div>
                   ))}
                 </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="space-y-6">
                <div className="p-8 bg-slate-900/50 rounded-[2.5rem] border border-white/5 h-full flex flex-col justify-center">
                   <h5 className="text-[12px] font-black text-slate-500 uppercase tracking-[0.5em] mb-4 text-center">ELITE ENGLISH MODE</h5>
                   <p className="text-center text-slate-400 font-serif italic">Veo3.1 Optimized Pipeline Active</p>
                </div>
              </div>
              <div className="space-y-6">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.8em] ml-6 opacity-50 italic">Scene Clip to Recreate</label>
            ...
            <div className="flex flex-col sm:flex-row gap-8 pt-8">
              ...
              >
                {state.exactSceneScript ? 'Scene Analyzed' : 'Analyze Scene Beat'}
              </Button>
              <Button className="flex-1 py-8 md:py-10 text-2xl md:text-3xl font-black rounded-[3rem] bg-white text-slate-950 hover:bg-slate-200 font-serif italic tracking-tighter shadow-3xl" disabled={!state.sceneVideo || !state.exactSceneScript} onClick={executeEliteProtocol}>GENERATE VEO PROMPTS</Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-16 animate-fadeIn pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start border-b border-white/5 pb-10 gap-6">
              <div className="space-y-4">
                <h3 className="text-6xl md:text-8xl font-black text-white font-serif tracking-tighter uppercase italic gold-glow leading-none">Veo3.1 Blueprint</h3>
                <div className="flex flex-wrap gap-4">
                  ...
                  <span className="px-6 py-2 bg-indigo-600/20 text-indigo-400 rounded-full text-[10px] font-black tracking-widest uppercase border border-indigo-500/30">ELITE ENGLISH</span>
                </div>
              </div>
              <div className="flex gap-4">
                <button onClick={() => copyToClipboard(state.result!.veoPrompt)} className="px-10 py-4 bg-yellow-600 text-black font-black uppercase text-xs rounded-full hover:bg-yellow-500 transition-all tracking-widest italic font-serif shadow-xl">Copy Veo Prompt</button>
              </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
               <div className="lg:col-span-8 space-y-12">
                 {/* Veo3.1 Master Prompt View */}
                 <div className="bg-black/90 p-8 md:p-12 rounded-[3.5rem] border border-white/10 shadow-[0_50px_100px_-20px_rgba(0,0,0,1)] flex flex-col">
                    <div className="flex justify-between items-center mb-10">
                       <h5 className="text-[12px] font-black text-yellow-600 uppercase tracking-[1em] italic flex items-center gap-4">
                        ...
                        VEO3.1 GENERATION PROMPT
                       </h5>
                    </div>
                    <div className="bg-slate-950/40 p-8 md:p-12 rounded-[2.5rem] border border-white/5 overflow-y-auto custom-scrollbar">
                       <pre className="text-yellow-100 text-lg md:text-xl font-mono whitespace-pre-wrap leading-[1.8]">
                         {state.result.veoPrompt}
                       </pre>
                    </div>
                 </div>
                 ...
                 <div className="bg-slate-950/40 p-12 rounded-[3rem] border border-white/5 space-y-4 shadow-2xl">
                    <h4 className="text-[10px] font-black text-yellow-600 uppercase tracking-[0.8em] italic">Directing Philosophy</h4>
                    <p className="text-xl font-serif italic text-white/90 leading-relaxed">{state.result.breakdown.transcreationPhilosophy}</p>
                 </div>

                 {/* Performance Master Class View (Secondary) */}
                 <div className="bg-slate-900/40 p-8 md:p-12 rounded-[3.5rem] border border-white/5 flex flex-col h-[500px]">
                    <div className="flex justify-between items-center mb-6">
                       <h5 className="text-[12px] font-black text-slate-500 uppercase tracking-[1em] italic">
                        ACTOR / PERFORMANCE NOTES
                       </h5>
                       <button onClick={() => copyToClipboard(state.result!.masterPrompt)} className="text-[10px] uppercase tracking-widest text-slate-400 hover:text-white">Copy Notes</button>
                    </div>
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                       <pre className="text-slate-400 text-sm font-mono whitespace-pre-wrap leading-[1.8]">
               ...
               <div className="lg:col-span-4 space-y-8">
                  ...
                  {/* Grounding Panel */}
                  <div className="bg-slate-950/60 p-10 rounded-[3.5rem] border border-white/5 space-y-6 shadow-2xl">
                    <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-[1em] italic opacity-60">Elite Research</h5>
                    <div className="space-y-4">
      ...
      <footer className="mt-24 mb-10 text-slate-800 text-[11px] font-black uppercase tracking-[3em] opacity-10 text-center px-4">
        <span>AL-NOKHBA VEO DIRECTOR v7.0.0</span>
      </footer>
    </div>
  );
};
